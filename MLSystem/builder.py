...
class ExperimentBuilder:
    """
    Hydraの設定(config)を受け取り、実験に必要なオブジェクト(Model, DataModule)を
    ...
    def build(self) -> ExperimentContext:
        ...
        datamodule = modules["DataModuleClass"](
            adapter_transform=input_transform, **all_params
        )
    ...
    def _load_definitions(self):
        ...
        try:
            return {
                "ModelClass": self.registry.get_main_class("models", config.model),
                "DataModuleClass": self.registry.get_main_class(
                    "datasets", config.dataset
                ),
                "adapter_mod": self.registry.get_adapter_module(
                    config.model, config.adapter
                ),
                "model_conf_cls": self.registry.get_config_class(
                    "models", config.model
                ),
                "adapter_conf_cls": self.registry.get_config_class(
                    "models", config.model, config.adapter
                ),
                "data_conf_cls": self.registry.get_config_class(
                    "datasets", config.dataset
                ),
                "common_conf_cls": find_config_class(
                    EnvManager().get_common_config_module()
                ),
            }
    ...
    def _extract_user_params(self):
        ...
        return {
            "common": (
                OmegaConf.to_container(config.common, resolve=True)
                if "common" in config
                else {}
            ),
            "model": (
                OmegaConf.to_container(config.model_params, resolve=True)
                if "model_params" in config
                else {}
            ),
            "adapter": (
                OmegaConf.to_container(config.adapter_params, resolve=True)
                if "adapter_params" in config
                else {}
            ),
            "data": (
                OmegaConf.to_container(config.data_params, resolve=True)
    ...
    def _compute_hash(self, modules, user_params):
        ...
        return compute_combined_hash(
            modules["common_conf_cls"],
            p_common,
            modules["model_conf_cls"],
            p_model,
            modules["adapter_conf_cls"],
            p_adapter,
            modules["data_conf_cls"],
            p_data,
        )
