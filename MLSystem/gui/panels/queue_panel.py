...
class QueuePanel(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        ttk.Button(control_frame, text="▶ Start", command=self.start_runner).pack(side=tk.LEFT, padx=2)
        ttk.Button(control_frame, text="⏹ Stop", command=self.stop_runner).pack(side=tk.LEFT, padx=2)
        ...
        ttk.Button(control_frame, text="⬆", width="auto", command=self.move_job_up).pack(side=tk.LEFT, padx=2)
        ttk.Button(control_frame, text="⬇", width="auto", command=self.move_job_down).pack(side=tk.LEFT, padx=2)
        ttk.Button(control_frame, text="🗑 Delete", command=self.delete_job).pack(side=tk.LEFT, padx=5)
        
        # Refresh (Right aligned)
        ttk.Button(control_frame, text="🔄 Refresh", command=self.update_list).pack(side=tk.RIGHT, padx=5)
    ...
    def _insert_row(self, data, status):
        ...
        self.tree.insert("", "end", iid=jid, values=(ttype, jid, status.upper(), model, dataset, dt), tags=(tag, status))
