...
class SettingsPanel(ttk.Frame):
    ...
    def setup_ui(self):
        ttk.Label(self, text="🚀 Experiment Settings", font=("", 14, "bold")).pack(pady=10)
        ...
        ttk.Radiobutton(task_frame, text="Train", variable=self.task_mode, value="train", command=self.update_action_state).pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(task_frame, text="Plot", variable=self.task_mode, value="plot", command=self.update_action_state).pack(side=tk.LEFT, padx=10)
    ...
    def update_form(self, event=None):
        ...
        if a_name:
            ...
            self.create_fields(lf_a, "models", m_name, a_schema, self.adapter_vars, "adapters", a_name)
    ...
    def get_params(self, var_dict):
        ...
        for k, (var, vtype) in var_dict.items():
            val = var.get()
            if vtype is int: params[k] = int(float(val)) if val else 0
            elif vtype is float: params[k] = float(val) if val else 0.0
            elif vtype is bool: params[k] = bool(val)
            else: params[k] = val
        return params
    ...
    def save_user_config(self, kind, name, var_dict, sub_kind=None, sub_name=None):
        if kind == "common": json_dir = "common"
    ...
    def submit_job(self):
        ...
        for k, v in c_p.items(): hydra_args.append(f"+common.{k}={v}")
        for k, v in m_p.items(): hydra_args.append(f"+model_params.{k}={v}")
