...
*   **実装**:
    *   `MLSystem/builder.py`: `ExperimentBuilder` クラスがHydraの設定(`cfg`)を受け取る。Hydraは `MLSystem` パッケージ内に配置された `config.yaml` をデフォルト設定として読み込み、CLIなどからの上書きを適用する。
...
│
├── configs/
│   └── registry.json         # パス管理レジストリ (プロジェクト固有)
│
...
| 目的          | 編集ファイル                                      | 内容                                                    |
| :---------- | :------------------------------------------ | :---------------------------------------------------- |
| **新規モデル追加** | `definitions/models/{name}/` + `registry.json` | ファイル作成後、**必ず `configs/registry.json` にパスを登録する。** 起動時のモデル指定は `python -m MLsystem.execute_train model=...` のようにCLIで行う。 `config.py` は `dataclass` で記述する。 |
...
システムの基本設定ファイル (`config.yaml`) は `MLSystem` パッケージ内にあり、通常は直接編集せず、コマンドライン引数で設定を上書きします。
