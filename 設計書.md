# 実験管理システム 設計書 (Experiment Management System Design Document) Ver 3.3
...
*   **実装**:
    *   `system/builder.py`: `ExperimentBuilder` クラスがHydraの設定(`cfg`)を受け取り、`Registry`を用いてクラスを取得。
...
*   **実装**:
    *   `system/checkpoint_manager.py`: チェックポイントの検索、一覧取得、再開パス決定、Callback作成のロジックを集約。
    *   `execute_train.py` は起動時に既存のチェックポイントが持つエポック数と、設定された `max_epochs` を比較する。
...
│   ├── builder.py            # ★新設: 実験環境構築 (Builderパターン)
│   ├── checkpoint_manager.py # ★新設: チェックポイント管理
│   ├── callbacks.py          # ★新設: ログ出力などのカスタムコールバック
│   ├── loader.py             # 実験の復元 (分析用)
...
2.  `system/submit.py` がJSONを作成し、`queue/pending/` に保存。

### B. 学習実行フロー (Execution Pipeline: Train)
Runnerが `system/execute_train.py` を呼び出す。
