...
4.  **Explicit Registry Management（明示的なレジストリ管理）**
    *   ファイルパスやモジュールの所在をディレクトリ構造から推測することを禁止する。
    *   全てのModel, Adapter, Datasetのパスは `configs/registry.json` で一元管理し、システム(`Registry`クラス)はこれを参照してロードを行う。
...
*   **実装**:
    ...
    *   **高度なジョブ管理**:
        ...
        *   **ログ確認機能**: ジョブ失敗時、詳細なエラーログを `queue/logs/` に保存し、GUIから参照可能にする（実装予定）。
...
│   └── registry.json         # ★新設: パス管理レジストリ
│
├── definitions/                  # 【ユーザー実装領域】
│   ├── datasets/
│   │   └── {dataset_name}/       # データセット定義
│   │       ├── config.py         # 設定 (dataclass定義)
...
│   ├── runner.py             # ジョブ実行ランナー
│   ├── registry.py           # ★新設: レジストリ管理・モジュールロード
│   ├── submit.py             # ジョブ登録
│   ├── hashing.py            # 設定ハッシュ計算
...
1.  **Path Resolution**: `configs/registry.json` を参照し、指定されたModel/Adapter/Datasetのファイルパスを特定。
2.  **Module Load**: `Registry` クラスを通じて、パスから直接モジュールと設定クラス(`dataclass`)をロード。
3.  **Adapter Binding**: Adapterから `input_transform` を取得し、Datasetを初期化。
4.  **Meta Info Transfer**: Datasetのメタ情報をAdapter経由でModel引数(`model_kwargs`)に変換。
5.  **Training**: PyTorch Lightning Trainerにより学習を実行し、チェックポイントを保存。
...
| **可視化の追加**  | 各階層の `plots.py` / `plot.py`                 | `BasePlot` を継承したクラスを作成。ファイル名は `registry.json` で指定したものと一致させる。            |
| **パラメータ追加** | 各 `config.py`                               | `dataclass` 内にフィールドを定義。GUI・ハッシュ計算に自動反映される。`CONFIG_SCHEMA` は不要。                  |