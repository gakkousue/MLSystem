...
class RCNPEvaluation(BasePlot):
    ...
    def get_predictions(self):
        ...
        if not ckpt_path:
            ...
            ckpt_path = self.loader.get_checkpoint_path()
            if not ckpt_path:
                raise RuntimeError("No checkpoint after training.")
    ...
    def execute(self):
        ...
        def calculate_roc_points(t, y_prob, signal_label, background_label):
            ...
            for th in thresholds:
                eff = np.sum(signal_probs >= th) / n_signal if n_signal > 0 else 0.0
                misid = (
                    np.sum(background_probs >= th) / n_background
                    if n_background > 0
                    else 0.0
                )
                effs.append(eff)
        ...
        for signal_idx in range(3):
            ...
            for background_idx in range(3):
                ...
                plt.plot(
                    effs[mask], misids[mask], label=f"BG: {class_names[background_idx]}"
                )
            ...
            save_path = os.path.join(
                self.loader.exp_dir, f"roc_{class_names[signal_idx]}.png"
            )
            plt.savefig(save_path)
            plt.close()
            print(f"Saved: {save_path}")
